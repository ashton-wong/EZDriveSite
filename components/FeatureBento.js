...
export default function FeatureBento({ heading, sub, benefitCards }) {
  ...
  const primary = benefitCards && benefitCards.length > 0 ? benefitCards[0] : null
  const secondary = benefitCards && benefitCards.length > 1 ? benefitCards.slice(1) : null
  ...
  return (
    <section ref={ref} className="py-32 bg-white text-black">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <p className="bento-sub mt-6 text-lg text-[#86868b] max-w-2xl">{sub || 'Engineered to go further and do more — every detail tuned to make a difference.'}</p>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-12 gap-8">
          <div className="feature-item md:col-span-7 rounded-3xl overflow-hidden bg-[#f5f5f7]">
            {primary ? (
              <div className="p-8">
                <h3 className="mt-2 text-3xl md:text-4xl font-semibold">{primary.title}</h3>
                <p className="mt-3 text-[#86868b]">{primary.description}</p>
              </div>
            ) : (
              <>
          ...
          <div className="md:col-span-5 grid grid-cols-1 gap-8">
            {secondary ? (
              secondary.map((b, i) => (
                <div key={i} className="feature-item rounded-3xl overflow-hidden bg-[#f5f5f7] h-40 md:h-56">
                  <div className="p-6">
                    <h4 className="text-2xl font-semibold">{b.title}</h4>
                    <p className="mt-2 text-[#86868b]">{b.description}</p>
                  </div>
                </div>
              ))
            ) : (
              <>
                <div className="feature-item rounded-3xl overflow-hidden bg-[#f5f5f7] h-40 md:h-56">
                  ...
                  <div className="p-6">
                    ...
                    <p className="mt-2 text-[#86868b]">Engineered to go further and do more — every detail tuned to make a difference.</p>
                  </div>
                </div>
